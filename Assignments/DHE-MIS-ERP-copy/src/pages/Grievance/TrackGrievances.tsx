...
export default function TrackGrievances() {
  ...
  // parse mm/dd/yyyy into Date (local)
  const parseDate = (str:any) => {
    if (!str) return null;
    // Accept either mm/dd/yyyy or yyyy-mm-dd (html date input)
  ...
  const filtered = useMemo(() => {
    ...
    return initialRows.filter((r) => {
      ...
      if (from || to) {
        const rowDate:any = parseDate(r.date);
        if (from && to) {
          // inclusive
      ...
      return (
        matchesSearch && matchesStatus && matchesCategory && matchesNature && matchesDate
      );
    });
  }, [filters, initialRows]);
  ...
  const stats = useMemo(() => {
    ...
    filtered.forEach((r) => {
      ...
      else if (r.status === "Under Investigation") tally["Under Investigation"]++;
    ...
    <div className={styles.page}>
      ...
      <div className={styles.card}>
        ...
        <div className={styles.filterActions}>
          <button className={styles.searchBtn} onClick={() => { /* UI-only, no API */ }}>
            🔍 Search
          </button>
      ...
      <div className={styles.card}>
        ...
        <p className={styles.cardSubtitle}>Found {filtered.length} grievance(s)</p>
        ...
        <div className={styles.tableFooter}>
          <div>Showing 1 to {Math.min(10, filtered.length)} of {filtered.length} results</div>
