...
export default function Research() {
  ...
  const filteredGuides = useMemo(() => {
    ...
    return researchGuides.filter((g) => {
      const matchDept = !filterDepartment || g.department === filterDepartment;
      const matchProgram =
        !filterProgramType || g.program === filterProgramType;
      ...
      return matchDept && matchProgram && matchStatus && matchSearch;
    });
  }, [filterDepartment, filterProgramType, filterStatus, search]);
    ...
    <div className={styles.page}>
      ...
      <div className={styles.filterBox}>
        ...
        <div className={styles.filtersRow}>
          ...
          <select value={filterProgramType} onChange={(e) => setFilterProgramType(e.target.value)}>
      ...
      <div className={styles.subTabs}>
        <button
          className={subTab === "researchGuides" ? styles.active_sub_tab : styles.subTab}
        ...
        <button
          className={subTab === "statistics" ? styles.active_sub_tab : styles.subTab}
      ...
      {subTab === "researchGuides" && (
        <div>
          <h3>Research Guides ({filteredGuides.length})</h3>
              ...
              {filteredGuides.length > 0 ? (
                filteredGuides.map((g, i) => (
                  <tr key={i}>
                    ...
                    <td>{g.department}</td>
                    <td>{g.subject}</td>
                    <td><span className={styles.tag}>{g.program}</span></td>
                    <td>{g.registered}</td>
                    <td>{g.completed}</td>
                    <td>{g.phd}/{g.foreign}</td>
                    <td>
                      <span className={styles.statusActive}>{g.status}</span>
                    </td>
                  </tr>
                ))
              ) : (
